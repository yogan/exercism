class ListOps {
    ...
    class func concat<T>(_ listOfLists: [[T]])-> [T] {
    ...
    class func filter<T>(_ list: [T], _ predicate: (T) -> Bool) -> [T] {
        if list.isEmpty {
            return []
        }
    ...
    class func map<T>(_ list: [T], _ op: (T) -> T) -> [T] {
        if list.isEmpty {
            return []
        }
    ...
    class func foldLeft<T>(_ list: [T], accumulated acc: T, combine op: (T, T) -> T) -> T {
        if list.isEmpty {
            return acc
        }
    ...
    class func foldRight<T>(_ list: [T], accumulated acc: T, combine op: (T, T) -> T) -> T {
        if list.isEmpty {
            return acc
        }
    ...
    class func rev<T>(_ list: [T], _ acc: [T]) -> [T] {
        if list.isEmpty {
            return acc
        }
